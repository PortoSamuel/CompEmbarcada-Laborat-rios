...
int process_touch(t_but botoes[], touchData touch, uint32_t n){
	for(int i = 0; i < n; i++){
		printf("%d %d %d %d\n", botoes[i].x, botoes[i].x + botoes[i].width, botoes[i].y, botoes[i].y + botoes[i].height);
...
void task_lcd(void){
  ...
  t_but but0 = {.width = 120, .height = 75, .colorOn = COLOR_YELLOW, .colorOff = COLOR_BLACK, .x = ILI9488_LCD_WIDTH/2, .y = 40, .status = 1, .callback = &but0_callback };
  t_but but1 = {.width = 120, .height = 75, .colorOn = COLOR_YELLOW, .colorOff = COLOR_BLACK, .x = ILI9488_LCD_WIDTH/2, .y = 140, .status = 1, .callback = &but1_callback };
  t_but but2 = {.width = 120, .height = 75, .colorOn = COLOR_YELLOW, .colorOff = COLOR_BLACK, .x = ILI9488_LCD_WIDTH/2, .y = 240, .status = 1, .callback = &but2_callback };
  ...
  while (true) {
    if (xQueueReceive( xQueueTouch, &(touch), ( TickType_t )  500 / portTICK_PERIOD_MS)) {
	  ...
	  int b = process_touch(buts, touch, 3);
	  printf("b: %d\n", b);
	  if (b >= 0) {
		  buts[b].callback(&buts[b]);
		  // buts[b].status = !buts[b].status;
		  // draw_button_new(buts[b]);
	  }
	  printf("x:%d y:%d\n", touch.x, touch.y);
    }
  }
